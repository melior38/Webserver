...
Http::Http(std::string &request/*, uint_ptr socket*/) : _request(request)
{
	this->parse_input();
	cout << "\n               CTRL_DATA\n" << endl;
	cout << this->getCtrlData()[0] << " " << this->getCtrlData()[1] << " " << this->getCtrlData()[2] << endl;
	cout << "\n               HEADERS\n" << endl;
...
void	Http::parse_input(void)
{
	...
	{
		this->setHeader(this->_request.substr(0));
		this->_body = "";
	}
}
...
void	Http::setCtrlData()
{
	...
	{
		std::string request_line = this->_request.substr(0, endline + 2);
		this->_request.erase(0, endline + 1);
		ss << request_line;
		for (int i = 0; i != 3; ++i)
			ss >> this->_ctrlData[i];
	}
